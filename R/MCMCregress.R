...
MCMCregress <- function(formula, data = list(), burnin = 1000, mcmc = 10000,
   ...
   if(mcmc %% thin != 0) {
      ...
      stop("\n Please respecify and call MCMCregress() again.\n") 
      } 
   if(mcmc < 0) {
   ...
   attr(output,"title") <- "MCMCpack Linear Regression Posterior Density Sample"
   return(output)

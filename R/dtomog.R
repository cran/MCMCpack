############################################################################
# produces temporally organized tomography plots (see King 1997)
# 
# Kevin M. Quinn
...
#
##########################################################################
dtomogplot <- function(r0, r1, c0, c1, time.vec=NA, 
  ...
  if (length(r0) != length(time.vec) & !is.na(time.vec)){
    stop("r0 and time.vec different lengths")
  }

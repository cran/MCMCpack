...
MCMCirt1d <- function(datamatrix, theta.fixed = 1, burnin = 500, mcmc = 1000,
   thin=5, verbose = FALSE, seed = 0, theta.start = NA, 
   alpha.start = NA, beta.start = NA, t0 = 0, T0 = 1, b0.alpha = 0, b0.beta = 0,
   B0.alpha = 1, B0.beta = 1, B0.corr = 0,
   store.item = FALSE, ... ) {
    
   # burnin / mcmc / thin error checking
   if(mcmc %% thin != 0) {
      cat("Gibbs interval not evenly divisible by thinning interval.")
      stop("\nPlease respecify and call MCMCirt1d () again.\n") 
      } 
   if(mcmc < 0) {
   ...
   if(sum(datamatrix==1 | datamatrix==0 | is.na(datamatrix)) != (J*K)) {
     cat("ERROR: Vote matrix contains elements other than 0, 1 or NA.\n")
   ...
   if(sum(B0.alpha > 0) != K) {
      cat("Some Elements of Vector of prior variances B0.alpha not conformable.")
      stop("\nPlease respecify and call MCMCirt1d() again.\n")         
   }
   if(is.null(dim(B0.beta))) {
      B0.beta <- B0.beta * matrix(1,K,1)  
   ...
   if((dim(B0.beta)[1] != K) || (dim(B0.beta)[2] != 1)) {
      cat("Vector of prior variances B0.beta not conformable.")
      stop("\nPlease respecify and call MCMCirt1d() again.\n")
      }
   if(sum(B0.beta > 0) != K) {
      cat("Some Elements of Vector of prior variances B0.beta negative.")
      stop("\nPlease respecify and call MCMCirt1d() again.\n")         
   }
   ...
   if(sum(B0.corr >= -1.0 & B0.corr<=1.0) != K) {
      cat("Some Elements of Vector of prior correlations B0.corr not defined.")
      stop("\nPlease respecify and call MCMCirt1d() again.\n")       
      }

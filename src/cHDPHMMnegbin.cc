//////////////////////////////////////////////////////////////////////////
// cHDPHMMnegbin.cc is C++ code to estimate a (sticky) HDP-HMM with 
// Negative Binomial emission distribution
...
#ifndef CHDPHMMNEGBIN_CC
#define CHDPHMMNEGBIN_CC
...
hmm_state negbin_hdp_reg_state_sampler(rng<RNGTYPE>& stream, 
                                       const int ns,
                                       const Matrix<>& Y,
                                       const Matrix<>& X,
                                       const Matrix<>& beta,
  ...
  for (int t = (n-1); t >= 0; --t){
    ...
    for (int j = 0; j< ns; ++j){
      py(j, t) += lngammafn(rho[j] + yt) - lngammafn(rho[j]) - lngammafn(yt +1);
      py(j, t) += rho[j] * log(rho[j]) + yt*log(lambda[j])- (rho[j] + yt) * log(rho[j] + lambda[j]);
    }
...
//////////////////////////////////////////// 
// HDPHMMnegbinReg implementation.  
//////////////////////////////////////////// 
template <typename RNGTYPE>
void HDPHMMnegbinReg_impl(rng<RNGTYPE>& stream, 
                          double *betaout, 
                          double *Pout, 
                          double *psout, 
                          double *sout,   
                          ...
                          const double *B0data)
{
  ...
  for (int iter = 0; iter < tot_iter; ++iter){
    ...
    //////////////////////
    if (ns > 1) {
      Sout = negbin_hdp_reg_state_sampler(stream, ns, Y, X, beta, P, nu, rho);
      s = Sout.s;
      nstate = Sout.nstate;
    } else {
      s = 1;
    ...
    if (debug > 0) Rprintf("Sampling rho...\n");
    //////////////////////
    // 5. Sample rho
    //////////////////////  
    ...
    if (debug > 0) Rprintf("Sampling nu...\n");
    //////////////////////
    // 6. Sample nu
    //////////////////////
    Matrix<> lambda(n, 1);
...
extern "C" {
  void cHDPHMMnegbin(double *betaout, 
                     double *Pout, 
                     double *psout, 
                     double *sout,   
                     double *nuout,
    ...
    MCMCPACK_PASSRNG2MODEL(HDPHMMnegbinReg_impl, 
                           ...
                           gammaout, akout, thetaout, rhosizes,
                           Ydata, Yrow, Ycol, 
                           Xdata, Xrow, Xcol, 
                           ...
                           a_theta, b_theta, e, f, g, rhostepdata, 
                           b0data, B0data);
  }//end MCMC
} // end extern "C"

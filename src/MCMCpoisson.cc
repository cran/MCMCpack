...
double poisson_logpost(Matrix<double>& Y, const Matrix<double>& X, 
		       const Matrix<double>& beta,
		       const Matrix<double>& beta_prior_mean, 
		       const Matrix<double>& beta_prior_var){
  
  // likelihood
  Matrix<double> link = X * beta;   
  double loglike = 0;
  for (int i=0; i<Y.rows(); ++i) {
    loglike += -::exp(link[i]) + Y[i] * link[i] - lngammafn(Y[i] + 1);
  }
...
extern "C"{
  ...
  void
  poissonpost (double* samdata, const int* samrow, const int* samcol, 
    ...
    double logpost_cur = poisson_logpost(Y,X,beta,b0,beta_prior_var);
    ...
    for (int iter = 0; iter < tot_iter; ++iter){
      ...
      double logpost_can = poisson_logpost(Y,X,beta_can, b0, beta_prior_var);
	...
	cout << "MCMCpoisson iteration = " << iter + 1 << endl;

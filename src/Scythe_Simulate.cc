/* Scythe_Simulate.cc
 *
 * This file provides implementations for random number generators,
...
namespace SCYTHE {
  ...
  /* The Marsaglia Generator KISS + SWB */
   ...
   *
   * I've left the original comments in to give credit in appropriate
   * places.
   */
  ...
  double rnchypgeom(const double& m1, const double& n1, 
    ...
    /*
      if (m1 <= 0) {
      cout << "Error ????: m1 <= 0 in SCYTHE::"
      << "dnychypgeom()" << endl;
      exit (500000);
      }
      if (n1 <= 0) {
      cout << "Error ????: n1 <= 0 in SCYTHE::"
      << "dnychypgeom()" << endl;
      exit (500000);
      }
      if (n2 <= 0) {
      cout << "Error ????: n2 <= 0 in SCYTHE::"
      << "dnychypgeom()" << endl;
      exit (500000);
      }
      if (psi <= 0) {
      cout << "Error ????: psi <= 0 in SCYTHE::"
      << "dnychypgeom()" << endl;
      exit (500000);
      }
    */
  ...
  /* Random Numbers */
  int
  rbinom (const int& n, const double& p)
  {
    static int report;
    int count = 0;
    double hold;
  ...
  /* Random Numbers */
  double
  rf (const double &n1, const double &n2)
  {
    if (n1 <= 0.0 || n2 <= 0.0)
      throw scythe_invalid_arg (__FILE__, __PRETTY_FUNCTION__,
				__LINE__, "n1 or n2 <= 0");
  ...
  /* Random Numbers */
  double
  rlnorm (const double &logmean, const double &logsd)
  {
    if (logsd < 0.0)
      throw scythe_invalid_arg (__FILE__, __PRETTY_FUNCTION__,
  ...
  /* Random Numbers */
  double
  rnbinom (const double &n, const double &p)
  {
    if (n <= 0 || p <= 0 || p > 1)
      throw scythe_invalid_arg (__FILE__, __PRETTY_FUNCTION__,
  ...
  /* Random Numbers */
  double
  rnorm (const double &mu, const double &sigma)
  {
    if (sigma <= 0) {
      throw scythe_invalid_arg (__FILE__, __PRETTY_FUNCTION__,
...
#undef swap_tail
  ...
  Matrix<int>
  rpois (const int &rows, const int &cols, const double &lambda)
  {
    int size = rows * cols;
    if (size <= 0) {
      throw scythe_invalid_arg (__FILE__, __PRETTY_FUNCTION__,
  ...
  /* Random Numbers */
  double
  rweibull (const double &shape, const double &scale)
  {
    if (shape <= 0 || scale <= 0)
      throw scythe_invalid_arg (__FILE__, __PRETTY_FUNCTION__,
				__LINE__, "shape or scale <= 0");
  ...
  /* Wishart */
  Matrix<double>
  rwish(const int &v, const Matrix<double> &S)
  {
    if (! S.isSquare()) {
  ...
  /* Dirichlet generator */
  Matrix<double> rdirich(const Matrix<double> &alpha) {
  ...
  /* Multivariate Normal */
  Matrix<double>
  rmvnorm(const Matrix<double> &mu, const Matrix<double> &sigma) {  
    int dim = mu.rows();
    if (mu.cols() != 1) {
      throw scythe_dimension_error(__FILE__, __PRETTY_FUNCTION__,
